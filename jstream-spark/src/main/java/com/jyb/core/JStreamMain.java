...
public class JStreamMain {
    ...
    private Dataset<Row> doSql(SparkSession spark, List<SqlEntry> sqlEntryList) {
        ...
        for (SqlEntry sqlEntry : sqlEntryList) {
            ...
            WaterMarkConfig waterMark = sqlEntry.getWaterMark();
            if (StringUtils.isNotBlank(alias) && !waterMark.isEmpty()) {
                spark.sql(sql).withWatermark(waterMark.getFiledName(), waterMark.getExpression()).createOrReplaceTempView(alias);
            } else if (StringUtils.isNotBlank(alias) && waterMark.isEmpty()) {
