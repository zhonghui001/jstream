...
public class MockConfiguration {
    ...
    public static JstreamConfiguration newInstance(){
        ...
        Config sink = new ConsoleSink.ConsoleSinkConfig(new OutPutModeConfig("complete"),
                new TriggerConfig(),null,null);

        JstreamConfiguration conf = new JstreamConfiguration(resouceConfig,extConfig,source,sink);

        conf.addSqlEntry(new SqlEntry("select get_json_object(value,'$.dbTable') dbtable,get_json_object(value,'$.update_time') update_time  from topic","topic2"))
                .addSqlEntry(new SqlEntry("select count(1) from topic2",""));
